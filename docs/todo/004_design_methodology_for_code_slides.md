...
**Status:** closed 2026-05-27 — §A + §B + §C + §D + §E + §F + §G all shipped.
...
- **2026-05-27 (§F shipped)** — reference design corpus checked in at
  `packages/skills/paper-deck/reference_corpus/`. 10 exemplar PNGs (one per
  canonical pattern) + `manifest.json` recording the pattern name, content
  used, and do / don't notes. Generated by
  `packages/skills/paper-deck/reference_corpus/generate.py`, which runs
  each canonical example through the same `export_deck_to_pptx` → soffice
  → PyMuPDF pipeline an agent uses (so the exemplars are pixel-identical
  to what the agent will produce). paper-deck SKILL §5d documents the
  recommended flow: pick pattern → read manifest → Read PNG → mirror
  the do, avoid the dont. Manifest schema validated by
  `test_reference_corpus_manifest_well_formed` (every pattern has file
  + do + dont; every referenced PNG exists; valid PNG header).
