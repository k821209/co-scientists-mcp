"""Reference materials (project-level): upload, list, download, delete, isolation."""
...
from co_scientist_local.tools import materials
...
def test_add_material_uploads_blob_and_doc(state, backend, tmp_path):
    ...
    m = materials.add_material(state, local_path=local, description="raw data")
    assert m["filename"] == "data.csv"
    assert m["content_type"] == "text/csv"
    assert m["size_bytes"] == 8
    assert m["description"] == "raw data"
    assert m["blob_path"].startswith("projects/p-alice/materials/")
    assert backend.get_blob(m["blob_path"]) == b"a,b\n1,2\n"


def test_add_material_missing_file_raises(state, tmp_path):
    with pytest.raises(FileNotFoundError):
        materials.add_material(state, local_path=str(tmp_path / "nope.pdf"))


def test_add_material_sanitizes_filename(state, tmp_path):
    local = _file(tmp_path, "weird name (1).pdf", b"%PDF-1.4")
    m = materials.add_material(state, local_path=local)
    assert m["filename"] == "weird_name_1_.pdf"
    assert m["content_type"] == "application/pdf"


def test_list_materials_newest_first(state, backend, tmp_path):
    a = _file(tmp_path, "a.txt", b"a")
    b = _file(tmp_path, "b.txt", b"b")
    m1 = materials.add_material(state, local_path=a)
    m2 = materials.add_material(state, local_path=b)
    backend.update_doc(
        f"projects/p-alice/materials/{m1['material_id']}",
        {"created_at": "2026-01-01T00:00:00.000000Z"},
    )
    backend.update_doc(
        f"projects/p-alice/materials/{m2['material_id']}",
        {"created_at": "2026-02-01T00:00:00.000000Z"},
    )
    out = materials.list_materials(state)
...
def test_get_material_writes_to_disk(state, tmp_path):
    ...
    m = materials.add_material(state, local_path=src)
    dest = tmp_path / "out"
    res = materials.get_material(state, m["material_id"], dest_dir=str(dest))
...
def test_get_material_respects_dest_path(state, tmp_path):
    ...
    res = materials.get_material(state, m["material_id"], dest_path=str(target))
...
def test_get_missing_material_raises(state):
    with pytest.raises(NotFound):
        materials.get_material(state, "deadbeef")
...
def test_delete_material_removes_blob(state, backend, tmp_path):
    ...
    assert materials.delete_material(state, m["material_id"]) is True
    assert backend.get_blob(m["blob_path"]) is None
    assert materials.delete_material(state, m["material_id"]) is False


def test_material_isolation_between_projects(state, other_state, tmp_path):
    src = _file(tmp_path, "secret.txt", b"alice")
    m = materials.add_material(state, local_path=src)
    # other_state is a different project → can't see alice's material.
    with pytest.raises(NotFound):
        materials.get_material(other_state, m["material_id"])
    assert materials.list_materials(other_state) == []
