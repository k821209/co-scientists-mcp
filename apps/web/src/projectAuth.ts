...
const projectAppCache = new Map<string, Promise<FirebaseApp>>();


async function _buildProjectApp(pid: string): Promise<FirebaseApp> {
  ...
  const apiKey = (snap.data() as { api_key?: string })?.api_key;
  if (!apiKey) {
 ...
 */
export async function getProjectStorage(pid: string): Promise<FirebaseStorage> {
...
export async function downloadProjectBlobAsText(pid: string, path: string): Promise<string> {
